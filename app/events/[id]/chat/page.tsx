...
export default function EventChatPage() {
  ...
  async function loadMessages() {
    ...
    if (!token) {
      setPageMessage("Please login first to view this event chat.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setPageMessage(data.message || "Could not load event chat.");
        return;
      }
      ...
      <section className="mx-auto flex min-h-screen max-w-5xl flex-col px-5 py-8 lg:px-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm font-bold text-zinc-500">
                Event #{eventId} • Group chat for joined members
              </p>
            </div>
        ...
        <div className="flex flex-1 flex-col rounded-[2.5rem] bg-white p-5 shadow-sm">
          <div className="flex-1 space-y-4 overflow-y-auto rounded-[2rem] bg-[#f6f8f5] p-4">
            {loading ? (
              <p className="p-5 font-black">Loading event chat...</p>
            ) : messages.length === 0 ? (
          ...
          <div className="mt-5 flex gap-3">
            <input
              ...
              placeholder="Write to the event group..."
